...
//		System.out.println(arr[4]);
		...
		System.out.println("배열에 끝 방까지의 합"+ sum );
		System.out.println("----------------------------------------");
	...
	public void useArray2() {
		// 영어 대문자 A~Z까지를 char형으로 배열을 만들고 0번째방 부터 추가 후 출력.
		//1. 선언)
		char[] alpha=null;
		//2. 생성)
		alpha=new char[26];
		//3.값 할당
		for(int i= 0 ; i < alpha.length ;i++) { //0 , 1, 2, 3,4 .. 25
			alpha[i]=(char)(i+65);
			//4. 값 사용
		...
		//3.값 할당
		for(int i= 0 ; i < lowerCase.length ;i++) { //0 , 1, 2, 3,4 .. 25
	...
	public void useArray3() {
		...
		System.out.printf("평균 %.1f점",(float)totalScore/javaScore.length);//평균은 소수점 이하 1자리까지만 출력.
		
	}//useArray3
	public void useArray4() {
		String[] addr= {"서울시 강남구 역삼동","서울시 동대문구 동대문동",
				"서울시 구로구 대림동"
				,"서울시 중구 필동","경기도 수원시 인계동","인천시 마계동",
				"제주특별자치도 이도동","서울시 동작구 상도동"};
		//모든 주소를 출력.
		int seoulCnt=0;
...
//		new TestArray1().useArray();// chain문법
