...
<!-- 파일 업로드 폼 -->
<!-- 파일을 업로드하기 위해서는 파라메터 전송방식에서 파일전송방식으로 변환해야한다.
   파라메터 전송방식 : enctype="application/x-www-form-urlencoded" 은
     Form Control의 값을 back-end에서 getParameter 로 받을 수 있다.
   파일 전송 방식 : enctype="multipart/form-data" 이고 Form Control의 값을 back-end에서
      getParameter로 받을 수 없다.
 -->
 
 <!-- RFC 1867 HTML Form 기반의 file upload -->
 <a href="http://localhost/jsp_prj/day1012/file_list.jsp">파일목록</a>
 <form action="upload_form_process.jsp" enctype="multipart/form-data" method="post">
 <label>업로더</label>
